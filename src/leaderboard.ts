...
export function calculateLeaderboardScore(input: ScoreInput): number {
  ...
  const diffMultiplier = input.difficulty === 'mogul' ? 1.5 : input.difficulty === 'indie' ? 0.8 : 1.0;
...
export function generateRunCard(entry: LeaderboardEntry): string {
  ...
  const diffBadge = entry.difficulty === 'mogul' ? '💀 Mogul' : entry.difficulty === 'indie' ? '🎥 Indie' : '🎬 Studio';
